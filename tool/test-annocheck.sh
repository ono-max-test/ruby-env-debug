...
if [ -z "${CI-}" ]; then
  ...
  # Build with volume option on local (non-CI).
  "${DOCKER}" build --rm -t "${TAG}" ${TOOL_DIR}/annocheck/
else
  # TODO: Temporary workaround on CI to build by copying binary files from host
  # to container without volume option, as I couldn't find a way to use volume
  # in container in container on GitHub Actions <.github/workflows/compilers.yml>.
  TAG="${TAG}-copy"
